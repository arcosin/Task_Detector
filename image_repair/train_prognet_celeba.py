...
def train(model, batch_size, epochs, device, transform_method, trainDir, modelDir, outDir, bs, lr, kl_weight=3e-5):
    global g_mu, g_var
    lossDict = {"rec": [], "kl": [], "all": []}
    col = model.addColumn()
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            save_image(x_concat, os.path.join(outDir, 'prognet-col-%s-reconst-%d.png' % (col, epoch+1)))
    torch.save(model.state_dict(), os.path.join(modelDir, "prognet_model_%s.pt" % transform_method))
    with open(os.path.join(modelDir, "prognet_losses_%s.json" % transform_method), 'w') as jsonfile:
...
def main(args):
    ...
    #train(model, args.batch_size, args.epochs, device, "colorize", args.train_dir, args.save_dir, args.output_dir, args.batch_size, args.lr)
    model = loadPrognet(model, ["denoise", "colorize"], "/scratch/gilbreth/jacobs57/Task_Detector/run_8-29-2021/models/prognet_model_colorize.pt")
    model.freezeAllColumns()
    ...
    start = time.time()
    train(model, args.batch_size, args.epochs, device, "inpaint", args.train_dir, args.save_dir, args.output_dir, args.batch_size, args.lr, kl_weight=3e-6)
    end = time.time()
    print("Time:   ", end - start)
    model.freezeAllColumns()
    start = time.time()
    train(model, args.batch_size, args.epochs, device, "perspective", args.train_dir, args.save_dir, args.output_dir, args.batch_size, args.lr)
    end = time.time()
    print("Time:   ", end - start)
    model.freezeAllColumns()
    start = time.time()
    train(model, args.batch_size, args.epochs, device, "vflip_to_hflip", args.train_dir, args.save_dir, args.output_dir, args.batch_size, args.lr)
    end = time.time()
    print("Time:   ", end - start)
    model.freezeAllColumns()
    start = time.time()
    train(model, args.batch_size, args.epochs, device, "invert_flip", args.train_dir, args.save_dir, args.output_dir, args.batch_size, args.lr)
    end = time.time()
    print("Time:   ", end - start)
    model.freezeAllColumns()
    start = time.time()
    train(model, args.batch_size, args.epochs, device, "edge_double_flip", args.train_dir, args.save_dir, args.output_dir, args.batch_size, args.lr)
    end = time.time()
    print("Time:   ", end - start)
    model.freezeAllColumns()
    #train(model, args.batch_size, args.epochs, device, "doom", args.train_dir, args.save_dir, args.output_dir, args.batch_size, args.lr)
    print("Training done.")
