...
class TaskDetector:
    def __init__(self, anomalyDetectorGen, savePath, distroMemSize = 100, detectorCache = -1):
        ...
        self.lastScores = dict()
        self.lastNScores = dict()
    ...
    def detect(self, state):
        ...
        with torch.no_grad():
            for name in self.taskMap.keys():
                ...
                self.lastScores[name] = score
                normScore = (score - distro[0]) / distro[1]   # Normalize.
                self.lastNScores[name] = normScore
                if bestScore < score:
                    task = name
                    bestScore = score
    ...
    def getLastScores(self):
        return (self.lastScores, self.lastNScores)

    def to(self, device):
        for name in self.taskMap.keys():
            self.resetDetectorDevice(name, device)
